...
export function reducer(state = initialState, action: ActivityActions | ActivityFilterActions): State {
  ...
  switch (action.type) {
    ...
    case ActivityFilterActionTypes.FilterActivity: {
      return filterActivity(state, action);
    }
...
function applyFilters(activity: Activity, filters: Dictionary<ActivityFilter>): number[] {
  ...
  console.log('Performance: filter values by index took ' + (t1 - t0) + ' milliseconds.');
  return idSet;
}
 ...
 *
 * @param {State} state
 * @param {FilterActivity | AddActivityFilter} action
 * @returns {{selectedActivityId: string | null; activitySport: Array<string>; activitySubSport: Array<string>;
 * sidebarContent: ActivitySidebarType; unfilteredActivity: Activity; filteredIndices: number[]; tsLookup: Map<string,
 * number>; ids: string[] | number[]; entities: Dictionary<Activity>}}
 */
function filterActivity(state: State, action: FilterActivity | AddActivityFilter) {
  // In this implementation there is no need to pass in the specific filter as all filters are re-applied
  const filters: Dictionary<ActivityFilter> = action.payload.allFilters;
  let activity: Activity;

  // add new activity filter to dictionary so it will be applied here
  if (action.type === ActivityFilterActionTypes.AddActivityFilter) {
    const newFilter: ActivityFilter = action.payload.activityFilter;
    filters[newFilter.id] = newFilter;
    activity = state.entities[state.selectedActivityId];
  } else {
    activity = deepCopyActivity(state.unfilteredActivity);
  }
  ...
  // this could be optimized (new all-set is not needed every call)
